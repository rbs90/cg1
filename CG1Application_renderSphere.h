#ifndef CG1APPLICATION_RENDERSPHERE_H
#define CG1APPLICATION_RENDERSPHERE_H
#include "CGContext.h"
#include "CGMatrix.h"
...
{
	CGMatrix4x4 mvScaled=mv*CGMatrix4x4::getScaleMatrix(radius,radius,1);
	context->cgVertexAttribPointer(CG_POSITION_ATTRIBUTE, circleV);
	context->cgVertexAttribPointer(CG_COLOR_ATTRIBUTE,    circleC);
	context->cgUniformMatrix4fv(CG_ULOC_MODELVIEW_MATRIX,1,false,mvScaled.getDataPtr());
	context->cgDrawArrays(CG_LINES, 0, circlelines*2);
	// reset mv (assuming it was set before)
	context->cgUniformMatrix4fv(CG_ULOC_MODELVIEW_MATRIX,1,false,mv.getDataPtr());
...
{
	CGMatrix4x4 mvScaled=mv*CGMatrix4x4::getScaleMatrix(radius,radius,radius);
	context->cgUniformMatrix4fv(CG_ULOC_MODELVIEW_MATRIX,1,false,mvScaled.getDataPtr());
...
//--------------------------------------------------------------------------
#endif // CG1APPLICATION_RENDERSPHERE_H
