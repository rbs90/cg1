...
private:
	...
	const float *m_pVertexAttributePointer[CG_ATTRIBUTE_COUNT];
...
public:
	...
	void cgVertexAttribPointer(CGenum attribute, const float *data);
	...
	int cgGenTexture_cgBindTexture_cgTexImage2D(int width, int height, const unsigned char* data4ub);
	...
	void cgReadPixels(unsigned char* data) const;
...
private:
	...
	/// Pre-calculate triangle area and decide if triangle should be culled. Returns true if visible.
	bool m_cgTriangleSetup(const CGVec4 &vertex0Position, const CGVec4 &vertex1Position, const CGVec4 &vertex2Position, float& area);
	...
	bool m_cgFragmentClipping(const CGFragmentData& fragment);
	...
	bool m_cgFragmentZTest(const CGFragmentData& fragment);

	/// Blend the fragment color from the underlying framebuffer color. Returns true.
	void m_cgFragmentBlending(CGFragmentData& fragment);

	/// Write current fragment into fragment buffer.
	void m_cgFragmentWriteBuffer(const CGFragmentData& fragment);
};
